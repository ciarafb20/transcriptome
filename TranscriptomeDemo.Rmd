---
title: "Transcriptome Assignment"
author: "Ciara Blanco"
date: "`r Sys.Date()`"
output: pdf_document
---
...
# This code is creating a data frame with two columns, id and stage. 
```{r}
...
# This code is filtering a ballgown object to keep only the genes that show sufficient variability across samples. 
```{r}
...
# I chose the transcript gene-PA0001. 
```{r}
results_transcripts[results_transcripts$transcriptNames == "gene-PA0001", ]
```
## This transcript is giving information on the gene name, the transcript name, the feature, id, fc, pval, and qval.

# This code is doing statistical filtering on my transcript results.
```{r}
...
# This code is renaming the columns of my gene matrix. 
```{r}
...
## The above graph is plotting the transcript per gene against the frequency at which they appear. There are more genes with only one transcript than anything else. 

# create a plot of how similar the two replicates are for one another. We have two data sets, I can modify this code in another chunk to create a plot of the other set by looking at both stage data sets (planktonic and biofilm).
```{r}
...
## If the two data sets are similar, it shoes experimental consistency and that I am using reliable data. 
...
## The figure above compares the log2(Fold change) for Planktonic vs Biofilm against the frequency at which it appears. There is horizontal symmetry in the fold changes, which is good because asymmetry indicates bias. 
...
## The figure above is comparing the biofilm and planktonic stage data against the log2(FPKM+1) data. 